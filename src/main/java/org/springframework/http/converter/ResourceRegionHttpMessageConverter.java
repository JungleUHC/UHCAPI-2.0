...
/* 152 */     end = Math.min(end, resourceLength - 1L);
...
/*     */     
/*     */     try {
/* 161 */       StreamUtils.copyRange(in, outputMessage.getBody(), start, end);
/*     */     } finally {
/*     */       
/*     */       try {
/* 165 */         in.close();
...
/* 176 */     Assert.notNull(resourceRegions, "Collection of ResourceRegion should not be null");
/* 177 */     HttpHeaders responseHeaders = outputMessage.getHeaders();
/*     */     
/* 179 */     MediaType contentType = responseHeaders.getContentType();
/* 180 */     String boundaryString = MimeTypeUtils.generateMultipartBoundaryString();
/* 181 */     responseHeaders.set("Content-Type", "multipart/byteranges; boundary=" + boundaryString);
/* 182 */     OutputStream out = outputMessage.getBody();
/*     */     
/* 184 */     Resource resource = null;
/* 185 */     InputStream in = null;
/* 186 */     long inputStreamPosition = 0L;
/*     */     
/*     */     try {
/* 189 */       for (ResourceRegion region : resourceRegions) {
/* 190 */         long start = region.getPosition() - inputStreamPosition;
/* 191 */         if (start < 0L || resource != region.getResource()) {
/* 192 */           if (in != null) {
/* 193 */             in.close();
/*     */           }
/* 195 */           resource = region.getResource();
/* 196 */           in = resource.getInputStream();
/* 197 */           inputStreamPosition = 0L;
/* 198 */           start = region.getPosition();
/*     */         } 
/* 200 */         long end = start + region.getCount() - 1L;
/*     */         
/* 202 */         println(out);
/* 203 */         print(out, "--" + boundaryString);
/* 204 */         println(out);
/* 205 */         if (contentType != null) {
/* 206 */           print(out, "Content-Type: " + contentType);
/* 207 */           println(out);
/*     */         } 
/* 209 */         long resourceLength = region.getResource().contentLength();
/* 210 */         end = Math.min(end, resourceLength - inputStreamPosition - 1L);
/* 211 */         print(out, "Content-Range: bytes " + region
/* 212 */             .getPosition() + '-' + (region.getPosition() + region.getCount() - 1L) + '/' + resourceLength);
/*     */         
/* 214 */         println(out);
/* 215 */         println(out);
/*     */         
/* 217 */         StreamUtils.copyRange(in, out, start, end);
/* 218 */         inputStreamPosition += end + 1L;
/*     */       } 
/*     */     } finally {
/*     */       
/*     */       try {
...
/* 242 */     os.write(buf.getBytes(StandardCharsets.US_ASCII));
/*     */   }
/*     */ }


/* Location:              /home/altaks/Téléchargements/mumblelink-1.0-SNAPSHOT.jar!/org/springframework/http/converter/ResourceRegionHttpMessageConverter.class
